...
def main(args, config):
    ...
    tokenizer = MBartTokenizer.from_pretrained(config['model']['transformer'], src_lang='en_XX', tgt_lang='en_XX')
    ...
    if args.finetune:
        ...
        state_dict = torch.load(args.finetune, map_location='cpu', weights_only=False)
...
def evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion, config, UNK_IDX, SPECIAL_SYMBOLS,
    ...
    with torch.no_grad():
        ...
        for step, (src_input, tgt_input,_) in enumerate(metric_logger.log_every(dev_dataloader, 1, header)):
    ...
    with open(args.output_dir + f'/output/tmp_pres_{epoch}.txt', 'w') as f:
        for i in range(len(tgt_pres)):
            f.write(tgt_pres[i] + '\n')
    with open(args.output_dir + f'/output/tmp_refs_{epoch}.txt', 'w') as f:
