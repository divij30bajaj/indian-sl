...
class ImageCLIP(nn.Module):
    ...
    def forward(self, src_input):
        ...
        #mask = src_input["attention_mask"].float().to(x.device)  # [B,N]
...
class SLRCLIP(nn.Module):
    ...
    def forward(self, src_input, tgt_input): 
        ...
        all_image_features = gather_with_grad(image_features)  # [B_global, D]
        all_text_features  = gather_with_grad(text_features)   # [B_global, D]

        logit_scale = self.logit_scale.exp()
        logits_per_image = logit_scale * image_features @ all_text_features.t()
        logits_per_text = logit_scale * text_features @ all_image_features.t()

        B_local = image_features.shape[0]
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            rank = torch.distributed.get_rank()
            world_size = torch.distributed.get_world_size()
            B_global = B_local * world_size
            global_pos = rank * B_local + torch.arange(B_local, device=image_features.device)
        else:
            B_global = B_local
            global_pos = torch.arange(B_local, device=image_features.device)
        ...
        ground_truth[torch.arange(B_local, device=image_features.device), global_pos] = 1.0


        return logits_per_image, logits_per_text, ground_truth, frames_feature

    def get_features(self, src_input, tgt_input):
        video_features, _ = self.model_images(src_input)
        text_features, _ = self.model_txt(tgt_input)

        video_features = video_features / video_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
                                 
        # all_video_features = gather_with_grad(video_features) 
