...
def main(args, config):
    ...
    print(dev_data)
    dev_sampler = torch.utils.data.distributed.DistributedSampler(dev_data, shuffle=True)
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
    ...
    for step, (src_input, tgt_input, masked_tgt_input, _) in enumerate(
        ...
        if step % 10 == 0 and torch.cuda.is_available():
            ...
            print(f"[step {step}] allocated={alloc:.1f}MB reserved={reserved:.1f}MB")
...
def evaluate(args, dev_dataloader, model, model_without_ddp, criterion, config, epoch, UNK_IDX, SPECIAL_SYMBOLS,
    ...
    os.makedirs(getattr(args, "eval_dump_dir", "./eval_dumps"), exist_ok=True)
    out_path = os.path.join(
        getattr(args, "eval_dump_dir", "./eval_dumps"),
        f"dev_epoch{epoch+1:03d}_rank{rank}.jsonl"
    )
      ...
      with torch.no_grad():
          with torch.cuda.amp.autocast():
              for step, (src_input, tgt_input, masked_tgt_input, name_batch) in enumerate(
                  ...
                  B = logits_per_image.size(0)          # local batch size
                  Bg = logits_per_image.size(1)         # global batch size (often world_size * B)
                  rank = utils.get_rank() if hasattr(utils, "get_rank") else 0
                  
                  idx = torch.arange(B, device=logits_per_image.device, dtype=torch.long)
                  
                  # If logits are global, positives are at columns [rank*B : rank*B+B]
                  if Bg != B:
                      gt = idx + rank * B
                  else:
                      gt = idx
                      
                  logp_i2t = F.log_softmax(logits_per_image, dim=1)  # [B, Bg]
                  logp_t2i = F.log_softmax(logits_per_text, dim=1)   # [B, Bg] OR [Bg, B] depending on your model
                  
                  per_i2t = -logp_i2t[idx, gt]
                  per_t2i = -logp_t2i[idx, gt]   # works if logits_per_text is also [B, Bg]
                  per_pair = 0.5 * (per_i2t + per_t2i)
                  ...
                  per_i2t_cpu = per_i2t.detach().float().cpu().tolist()
                  per_t2i_cpu = per_t2i.detach().float().cpu().tolist()
                  per_pair_cpu = per_pair.detach().float().cpu().tolist()
                  
                  if torch.is_tensor(name_batch):
                      ids = name_batch.detach().cpu().tolist()
                  else:
                      ids = list(name_batch)
                  ...
                  for i in range(B):
                      rec = {
                          ...
                          "i_in_batch": int(i),
                          "sample_id": ids[i],
                          "loss_i2t": float(per_i2t_cpu[i]),
                          "loss_t2i": float(per_t2i_cpu[i]),
                          "loss_pair": float(per_pair_cpu[i]),
                          "masked_lm_loss_batch": float(masked_lm_loss.item()),
                      }
...
def extract_sign_features(args, dev_dataloader, model, output_file, split="dev"):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    image_features_dict, text_features_dict = {}, {}

    with torch.no_grad():
        for images, tgt_input in metric_logger.log_every(dev_dataloader, 20):
            video_features, text_features = model.get_features(src_input=images, tgt_input=tgt_input)
            video_features = video_features.cpu()
            text_features = text_features.cpu()
            
            for index, name in enumerate(images["name_batch"]):
                image_features_dict[name] = video_features[index]
                text_features_dict[name] = text_features[index]

    metric_logger.synchronize_between_processes()
    print("* Extracted features for dataset")
    path = f"./data/Phonexi-2014T/labels.{split}"
    raw_data = utils.load_dataset_file(path)
    datalist = []
    for name in raw_data.keys():
        item = raw_data[name]
        if name in image_features_dict.keys():
            item["video_embedding"] = image_features_dict[name]
            item["text_embedding"] = text_features_dict[name]
...
def main_extract_features(args, config):
    ...
    output_file = os.path.join(args.output_dir, f"{split}_extracted_features_{str(utils.get_rank())}")
